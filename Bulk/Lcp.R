...
# Creating a heatmap
png(filename = "Graphs_png/Lcp_xCell.png", 
    width = 7, 
    height = 7, 
    res = 450, 
    units = "in")

pheatmap(feature_counts_Lcp_tum_xCell,
         cluster_rows = TRUE,
         show_rownames = TRUE,
         border_color = NA,
         fontsize = 10,
...
png(filename = "Graphs_png/Lcp_CD19_vs_CD20.png", 
    width = 7, 
    height = 7, 
    res = 450, 
    units = "in")
...
res_table_Lcp_tum$diffexpressed[res_table_Lcp_tum$log2FoldChange > 1 & res_table_Lcp_tum$padj < 0.01] <- "Up-regulated"
# Set as "DOWN" if log2Foldchange < -1 and padj < 0.01
res_table_Lcp_tum$diffexpressed[res_table_Lcp_tum$log2FoldChange < -1 & res_table_Lcp_tum$padj < 0.01] <- "Down-regulated"
...
p_Lcp <- ggplot(res_table_Lcp_tum, aes(x = log2FoldChange, y = -log10(padj), colour = diffexpressed)) +
  ...
  ggtitle("G vs A(p_adj<0.01 & |LFC|>1)") +
...
# Save the plot
ggsave("Graphs_png/Lcp_volcano_plot.png", plot = p_Lcp, height = 10, width = 10)
...
png(filename = "Graphs_png/Lcp_heatmap.png", 
    width = 10, 
    height = 10, 
    res = 450, 
    units = "in")

tmp_4 <- pheatmap(rld_counts_sig_Lcp_tum,
                  ...
                  annotation_colors = list(isotype = c(IgA="chartreuse3", IgG="yellow1")),
                  fontsize = 10,
                  fontsize_row = 10, 
                  scale = "row",
                  angle_col = "315")
...
tmp_4@matrix_color_mapping@name <- " "
tmp_4
...
background_data <- data.frame(
  xmin = c(-Inf, -Inf, -Inf, -Inf),  # x-min for all rectangles, set to -Inf
  xmax = c(Inf, Inf, Inf, Inf),      # x-max for all rectangles, set to Inf
  ymin = c(0, cumulative_sums[1] + 0.5, cumulative_sums[2] + 0.5, cumulative_sums[3] + 0.5),  # y-min for each rectangle
  ymax = c(cumulative_sums[1] + 0.5, cumulative_sums[2] + 0.5, cumulative_sums[3] + 0.5, length(combined_results_trimmed$Method)),  # y-max for each rectangle
...
boxplot <- ggplot(norm_data_gg, aes(x = Var, y = Val, fill = isotype)) +
  ...
  theme_bw()
ggsave(filename = "Graphs_png/Lcp_boxplot_counts.png", plot = boxplot, device = "png", height=7, width=14)
...
IL5RA_dotplot <- ggplot(norm_data, aes(x=factor(patient), y=IL5RA, col = isotype)) +
  ...
  ggtitle("IL5RA")
ggsave(filename = "Graphs_png/Lcp_dotplot_counts_IL5RA.png", plot = IL5RA_dotplot, device = "png", height=7, width=7)
...
FCRL4_dotplot <- ggplot(norm_data, aes(x=factor(patient), y=FCRL4, col = isotype)) +
  ...
  theme(axis.text.x=element_text(angle=90, hjust=1),
        ...
        plot.title = element_text(hjust = 0.5)) +
  ggtitle("FCRL4") 
ggsave(filename = "Graphs_png/Lcp_dotplot_counts_FCRL4.png", plot = FCRL4_dotplot, device = "png", height=7, width=7)
...
RUNX2_dotplot <- ggplot(norm_data, aes(x=factor(patient), y=RUNX2, col = isotype)) +
  ...
  theme(axis.text.x=element_text(angle=90, hjust=1),
        ...
        plot.title = element_text(hjust = 0.5)) +
  ggtitle("RUNX2") 
ggsave(filename = "Graphs_png/Lcp_dotplot_counts_RUNX2.png", plot = RUNX2_dotplot, device = "png", height=7, width=7)
...
PDCD1_dotplot <- ggplot(norm_data, aes(x=factor(patient), y=PDCD1, col = isotype)) +
  ...
  theme(axis.text.x=element_text(angle=90, hjust=1),
        ...
        plot.title = element_text(hjust = 0.5)) +
  ggtitle("PDCD1") 
ggsave(filename = "Graphs_png/Lcp_dotplot_counts_PDCD1.png", plot = PDCD1_dotplot, device = "png", height=7, width=7)
...
RANKL_dotplot <- ggplot(norm_data, aes(x=factor(patient), y=TNFSF11, col = isotype)) +
  ...
  theme(axis.text.x=element_text(angle=90, hjust=1),
        ...
        plot.title = element_text(hjust = 0.5)) +
  ggtitle("RANKL") 
ggsave(filename = "Graphs_png/Lcp_dotplot_counts_RANKL.png", plot = RANKL_dotplot, device = "png", height=7, width=7)
